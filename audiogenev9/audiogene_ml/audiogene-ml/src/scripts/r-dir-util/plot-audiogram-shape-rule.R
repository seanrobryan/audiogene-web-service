...
rule_dir <- file.path(output_dir, paste0("rule_", rule_num, "_bin_size_", age_bin_size))
...
# t = "Threshold Ratio - step_size=.2 (Rule 3) Audioprofile - mean"
# t <- "Threshold - min:25, max:10 flat:10"
# ext <- "threshold_rule_2_5_flat_min_max_25_10_10_30"
# file1 <- paste0("../model_results_test/shape_audioprofiles_rule_individual_2_", ext, ".html")
# file2 <- paste0("../model_results_test/shape_audioprofiles_majority_rule_group_2_", ext, ".html")
# file3 <- paste0("../model_results_test/df_rule_subset_2_", ext, ".csv")
...
file4 <- file.path(analysis_dir, "gene_annotatated_age_group_")
...
if(rule_num == 1) {
  ...
  df_shape <- rule_2(df_shape, low, medium, high, threshold_1)
...
# Write gene_annotatied.csv for different age groups
# groups <- as.data.frame(unique(df_shape['age_group']))
# o <- apply(groups, MARGIN = 1, function (group) df_shape %>% select(age_group, locus, shape) %>%
#   filter(age_group == group) %>% write.csv(paste0(file4, group, '.csv'), row.names = FALSE))
